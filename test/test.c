...
static bool test_read_file(char* path, uint8_t* expected_data, int size)
{
    SceUID fd = sceIoOpen(path, SCE_O_RDONLY, 0777);
    if (fd < 0) {
        perr("Could not open '%s': 0x%08X\n", path, fd);
    ...
    sceIoClose(fd);
    if (read != size) {
        perr("Could not read data from '%s': 0x%08X\n", path, read);
        return false;
    }
    for (int i = 0; i < size; i++) {
        if (data[i] != expected_data[i]) {
            perr("Data from '%s' differs at offset 0x%08X:\n", path, i);
...
int main()
{
    ...
    if (group[1]) {
        ...
        DISPLAY_TEST("Read 'sce_sys/package/work.bin'", test_read_file, "ux0:app/PCSE00001/sce_sys/package/work.bin", work_bin_data, sizeof(work_bin_data));
        DISPLAY_TEST("Read 'sce_sys/param.sfo'", test_read_file, "ux0:app/PCSE00001/sce_sys/param.sfo", param_sfo_data, sizeof(param_sfo_data));
    }
